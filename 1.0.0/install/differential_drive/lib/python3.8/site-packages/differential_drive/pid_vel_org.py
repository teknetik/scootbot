...
class PidVelocity(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"{self.nodename} started Test")
        ...
        self.create_subscription(Int16, 'lwheelenc', self.wheel_callback, 10)
    ...
    #def spin(self):
        ...
        self.then = self.get_clock().now()
        self.get_logger().info(str(rclpy.ok()))
        while rclpy.ok():
            self.get_logger().info("spinning")
            self.get_logger().info("spin once")
            self.get_logger().info(str(self.ticks_since_target))
            self.get_logger().info(str(self.timeout_ticks))
            ...
            self.vel = Float32()
            self.vel.data = 0.0
            self.get_logger().info("Spin once target get " + str(self.target))
            ...
            self.get_logger().info(str(self.r))
            self.r.sleep()
    ...
    def calc_velocity(self):
        ...
        self.get_logger().debug(str(type(self.dt_duration)))
        self.get_logger().debug(str(self.dt_duration.nanoseconds))
        self.get_logger().debug(str(round(self.dt_duration.nanoseconds / 1e+9, 2)))
        ...
        if self.wheel_latest == self.wheel_prev:
            ...
            if abs(cur_vel) < self.vel_threshold:
                ...
                if abs(cur_vel) < self.vel.data:
                    self.get_logger().debug("-D- %s cur_vel < self.vel.data" % self.nodename)
            ...
            self.wheel_prev = self.wheel_latest
            self.then = self.get_clock().now()
        self.get_logger().debug(str(self.vel))
        self.get_logger().debug(str(type(self.vel)))
    ...
    def do_pid(self):
        ...
        pid_dt = round(pid_dt_duration.nanoseconds / 1e+9, 2)
        #pid_dt = pid_dt_duration.to_sec()
        self.prev_pid_time = self.get_clock().now()

        self.error = self.target - self.vel.data
        self.integral = self.integral + (self.error * pid_dt)
        self.get_logger().debug("i = i + (e * dt):  %0.3f = %0.3f + (%0.3f * %0.3f)" % (self.integral, self.integral, self.error, pid_dt))
        self.derivative = (self.error - self.previous_error) / pid_dt
        self.previous_error = self.error

        self.motor.data = (self.Kp * self.error) + (self.Ki * self.integral) + (self.Kd * self.derivative)

        if self.motor.data > self.out_max:
            self.motor.data = self.out_max
            self.integral = self.integral - (self.error * pid_dt)
        if self.motor.data < self.out_min:
            self.motor.data = self.out_min
            self.integral = self.integral - (self.error * pid_dt)
        ...
        self.get_logger().debug("vel:%0.2f tar:%0.2f err:%0.2f int:%0.2f der:%0.2f ## motor:%d " %
                       (self.vel.data, self.target, self.error, self.integral, self.derivative, self.motor.data))
    ...
    def target_callback(self, msg):
        self.get_logger().debug("target_callback")
...
def main(args=None):
    ...
    try:
        ...
        pid_velocity.get_logger().debug("rclpy spin init")
